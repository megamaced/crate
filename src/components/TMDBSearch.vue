<template>
  <div class="enrichment-search">
    ...
    >
      No TMDB token saved — add one in Settings to enable film search.
    </p>
    <p
...
async function search() {
  ...
  results.value = []
  try {
    const res = await axios.get(generateOcsUrl('/apps/crate/api/v1/tmdb/search'), {
...
async function select(result) {
  // Fetch full details to get director etc.
  try {
    const res = await axios.get(generateOcsUrl(`/apps/crate/api/v1/tmdb/movie/${result.tmdbId}`))
