<template>
  <div class="enrichment-search">
    ...
    >
      No ComicVine API key saved — add one in Settings to enable comic search.
    </p>
    <p
...
async function search() {
  ...
  results.value = []
  try {
    const res = await axios.get(generateOcsUrl('/apps/crate/api/v1/comicvine/search'), {
    ...
    console.error('ComicVine search failed', e)
    showError('ComicVine search failed')
    searched.value = true
  } finally {
...
async function select(result) {
  // Fetch full volume details (description, artwork)
  try {
    const res = await axios.get(generateOcsUrl(`/apps/crate/api/v1/comicvine/volume/${result.comicVineId}`))
