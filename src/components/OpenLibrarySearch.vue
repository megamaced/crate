    ...
    >
      ...
      >
        ...
        <div class="enrichment-result-info">
          ...
          <span class="enrichment-result-artist">{{ result.artist }}</span>
          <span class="enrichment-result-meta">
            <template v-if="result.year">{{ result.year }}</template>
            <template v-if="result.label">, {{ result.label }}</template>
          </span>
        </div>
      </li>
    </ul>
...
async function search() {
  ...
  results.value = []
  try {
    const res = await axios.get(generateOcsUrl('/apps/crate/api/v1/openlibrary/search'), {
    ...
    showError('Open Library search failed')
    searched.value = true
  } finally {
...
async function select(result) {
  // Fetch full work details (description, cover, author bio)
  try {
    const encoded = encodeURIComponent(result.workKey.replace(/^\/works\//, ''))
    const res = await axios.get(generateOcsUrl(`/apps/crate/api/v1/openlibrary/work/${encoded}`))
    const work = res.data.ocs?.data ?? {}
    emit('select', { ...result, ...work })
  } catch {
    emit('select', result)
  }
  results.value = []
